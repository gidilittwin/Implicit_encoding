...
run_mode       = tf.Variable(0, name='run_node',dtype=tf.int32,trainable=False)
...
def evaluate(test_iterator, session, run_mode, config, test_dict, next_element):
    ...
    for epoch_test in range(config.im_per_obj):
        ...
        while True:
            try:
                ...
                accuracy_t_ ,iou_t_, iou_image_t, batch_ = session.run([test_dict['accuracy'], test_dict['iou'], test_dict['iou_image'], next_element],feed_dict=feed_dict) 
                acc_mov_avg_test = acc_mov_test.push(accuracy_t_)
                iou_mov_avg_test = iou_mov_test.push(iou_t_)
                classes.append(batch_['classes'])
                ids.append(batch_['ids'])
    ...
    return acc_mov_avg_test, mean_average_iou, classes, ids, ious
    ...
    while True:
        try:
            ...
            if step % config.test_every == config.test_every -1:            
                acc_test, iou_test, classes_test, ids_test, ious_test = evaluate(test_iterator, session, run_mode, config, test_dict, next_element)
                ...
                print('Training: epoch: '+str(epoch)+' ,avg_accuracy: '+str(acc_mov_avg)+' ,avg_loss: '+str(loss_mov_avg)+' ,IOU: '+str(iou_mov_avg))
                ...
                if iou_test>max_test_iou:
                    ...
                    np.save( directory+'/test_results', {'classes':Classes,'ids':Ids,'ious':Ious})
