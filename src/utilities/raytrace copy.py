...
class Raycast(object):
    ...
    def add_camera(self,camera_position=[-2., 0., 0.],lookAt=(0,0,0),focal_length=1,name='camera_1'):
        with tf.name_scope(name):
            with tf.variable_scope(name+'_ray',reuse=tf.AUTO_REUSE):
                Cam = Camera(self.resolution,camera_position,lookAt,focal_length)
    ...
    def eval_func(self,model_fn,args,step_size, epsilon, temp):
        ...
        for i in range(len(self.cameras)):
            ray_step,cloud,normal = self.cameras[i].evaluate(model_fn,args,step_size,epsilon,temp)
            ...
            images.append(image)
        return ray_steps,renders,clouds,normals,camera_resets,backgrounds,images
...
class Camera(object):
    def __init__(self,resolution,camera_position=[-2., 0., 0.],lookAt=(0.,0.,0.),focal_length=1):
    ...
    def parametrize_rays(self):
        self.t = tf.get_variable(name="ScalingFactor", shape=self.resolution, initializer=tf.zeros_initializer())
        ...
        self.coordinates = tf.stack((self.x,self.y,self.z),axis=0)
    ...
    def evaluate(self,eval_fn,args,step_size = 0.005, epsilon = 0.0001,temp = 1. ):
        ...
        self.evaluated_function = evaluated_function
        # Iteration operation
        ...
        self.point_cloud = tf.transpose(tf.reshape(self.coordinates,(3,-1)),(1,0))
        return self.ray_step,self.point_cloud,self.normal_vector
