...
def mlp(xyz, mode_node, theta, config):
    ...
    features = tf.tile(features,(1,1000,1,1))
    xyz  = tf.expand_dims(xyz,axis=2)
    inputs = tf.concat((xyz,features),axis=-1)
    with tf.variable_scope("Input"):
        conv0_w, conv0_b = CONV2D([1,1,config.embedding_size+3,2048])
        c0      = tf.nn.conv2d(inputs,conv0_w,strides=[1, 1, 1, 1],padding='SAME')
        current = tf.nn.bias_add(c0, conv0_b)
        current = tf.nn.relu(current)
        current = tf.concat((current,inputs),axis=-1)
    with tf.variable_scope("1"):
        conv0_w, conv0_b = CONV2D([1,1,2048+config.embedding_size+3,1024])
        c0      = tf.nn.conv2d(current,conv0_w,strides=[1, 1, 1, 1],padding='SAME')
        current = tf.nn.bias_add(c0, conv0_b)
        current = tf.nn.relu(current)
        current = tf.concat((current,inputs),axis=-1)
    with tf.variable_scope("2"):
        conv0_w, conv0_b = CONV2D([1,1,1024+config.embedding_size+3,512])
        c0      = tf.nn.conv2d(current,conv0_w,strides=[1, 1, 1, 1],padding='SAME')
        current = tf.nn.bias_add(c0, conv0_b)  
        current = tf.nn.relu(current)
        current = tf.concat((current,inputs),axis=-1)
    with tf.variable_scope("3"):
        conv0_w, conv0_b = CONV2D([1,1,512+config.embedding_size+3,256])
        c0      = tf.nn.conv2d(current,conv0_w,strides=[1, 1, 1, 1],padding='SAME')
        current = tf.nn.bias_add(c0, conv0_b)
        current = tf.nn.relu(current)
        current = tf.concat((current,inputs),axis=-1)
    with tf.variable_scope("4"):
        conv0_w, conv0_b = CONV2D([1,1,256+config.embedding_size+3,128])
        c0      = tf.nn.conv2d(current,conv0_w,strides=[1, 1, 1, 1],padding='SAME')
        current = tf.nn.bias_add(c0, conv0_b)
        current = tf.nn.relu(current)
        current = tf.concat((current,inputs),axis=-1)
    with tf.variable_scope("out"):
        conv0_w, conv0_b = CONV2D([1,1,128+config.embedding_size+3,1])
        c0      = tf.nn.conv2d(current,conv0_w,strides=[1, 1, 1, 1],padding='SAME')
