...
#STYLE_LAYERS = ('relu4_2','relu5_2')
kernel_s = 7
pad = int(np.floor(kernel_s/2.) )
...
def stylize(network, initial, initial_noiseblend, content, content_mask, styles, preserve_colors, iterations,
    ...
    with g.as_default(), g.device('/cpu:0'), tf.Session() as sess:
        ...
        for layer in CONTENT_LAYERS+STYLE_LAYERS:
            content_mask_features[layer] = net_mask[layer].eval(feed_dict={mask: np.expand_dims(np.expand_dims(content_mask,axis=0),axis=4)})
...
#            map_ = (net[content_layer] - content_features[content_layer])*(1.-content_mask_features[content_layer])
            loss_ = content_layers_weights[content_layer] * content_weight * (2 * tf.nn.l2_loss(map_) /
...
#        plt.pause(100)
        ...
        for i in range(len(styles)):
            ...
            for style_layer in STYLE_LAYERS:
                ...
                style_weights = np.reshape(content_mask_features[style_layer],(-1))
#                loss_ = tf.reduce_mean(tf.reduce_mean(tf.pow(layer_patches-style_filters_tf_gathered, 2),axis=1)*tf.stop_gradient(max_filter_response_weight))
                loss_ = tf.reduce_mean(tf.reduce_mean(tf.pow(layer_patches-style_filters_tf_gathered, 2),axis=1)*style_weights)
