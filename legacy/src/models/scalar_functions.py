...
def cell_2d_cnn(in_node,scope,mode,weights,bias,act=True,normalize=False):
    with tf.variable_scope(scope):
        if normalize==True:
...
def deep_sdf2(xyz, mode_node, theta):
    ...
    for ii in range(len(theta)):
        if ii<len(theta)-1:
            ...
            act=False
        image = cell_2d_cnn (image,   'l'+str(ii),mode_node,theta[ii]['w'],theta[ii]['b'],act=act,normalize=True) 
    
    grads = tf.gradients(image,xyz)[0]
    grads_norm = tf.sqrt(tf.reduce_sum(tf.square(grads),axis=2,keep_dims=True))
    image = image/grads_norm
    image = tf.nn.relu(image)
    
    return image    


def deep_sdf3(coordiantes, mode_node, theta):
    shape = coordiantes.get_shape().as_list()
    image = tf.reshape(coordiantes,(shape[0],-1,3))
    for ii in range(len(theta)):
        act=False
        image = cell_2d_cnn (image,   'l'+str(ii),mode_node,theta[ii]['w'],theta[ii]['b'],act=act,normalize=True) 
    
    image = tf.reduce_max(image,axis=-1,keep_dims=True)
    out = tf.reshape(image,(shape[0],shape[1],shape[2]))
    return out    
